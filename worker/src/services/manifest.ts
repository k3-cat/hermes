...
export async function getVer(c: Context<HonoCtx>, sub: string) {
	const ver = await c.env.KV_HRM.get(verKey(sub), { cacheTtl: KV_TEMP_TTL });
	if (!ver) {
...
export async function setVer(c: Context<HonoCtx>, sub: string, ver: string) {
	return await c.env.KV_HRM.put(verKey(sub), ver);
}

export async function getManifest(c: Context<HonoCtx>, sub: string) {
	const index = await c.env.KV_HRM.list({ prefix: manifestKey(sub, "") });

	return await c.env.KV_HRM.get(
		index.keys.map((item) => item.name),
		{ cacheTtl: KV_ALMOST_IMMUTABLE_TTL },
	);
}

export async function getManifestItemFor(
	c: Context<HonoCtx>,
	sub: string,
	platform: string,
) {
	const name = await c.env.KV_HRM.get(manifestKey(sub, platform), {
		cacheTtl: KV_ALMOST_IMMUTABLE_TTL,
	});
	if (!name) {
		throw new LogicalError(HttpStatus.NOT_FOUND, {
