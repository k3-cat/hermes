...
export async function getKeyOfLatestResourceFor(
	c: Context<HonoCtx>,
	sub: string,
	platform: string,
) {
...
export async function fetchResources(
	...
	const endpoint = await c.env.KV_HRM.get(`${sub}:endpoint`, {
...
export async function uploadResource(
	...
	const name = (await getManifestItemFor(c, sub, platform))?.replace(
		VERSION_PLACEHOLDER,
		ver,
	);
	const key = `${sub}/${name}`;
	const ret = await r2_upload(c.env.R2_STATIC, key, data, {
		...
		sha256: options?.sha256
			? Buffer.from(options.sha256, "hex").buffer
			: undefined,
	});
