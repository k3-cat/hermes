...
export class R2GetRet {
	...
	getHeaders(): Headers {
		...
		headers.set(
			HttpResponseHeader.CONTENT_DISPOSITION,
			`attachment; filename=${this.obj.key.split("/").pop()}`,
		);
		headers.set(
			HttpResponseHeader.LAST_MODIFIED,
			this.obj.uploaded.toUTCString(),
		);
		headers.set(HttpResponseHeader.ETAG, this.obj.httpEtag);
		const digest = Object.entries(this.obj.checksums)
			.filter(
				(item): item is [string, ArrayBuffer] => item[1] instanceof ArrayBuffer,
			)
			.map(
		...
		if (this.isPartial) {
			console.debug(
				`S:r2-gr - partial content (${this.offset}+${this.length}/${this.obj.size})`,
			);
			headers.set(
