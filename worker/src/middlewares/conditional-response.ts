...
class Condiction {
	...
	constructor(headers: Headers) {
		...
		if (this.ifMatch && this.ifNoneMatch) {
			throw new LogicalError(HttpStatus.BAD_REQUEST, {
				hrm: "M:cr",
				msg: "meaningless prerequisites",
			});
		}
	}
	...
	meetAllConditions(headers: Headers) {
		...
		if (lastModified) {
			...
			if (
				...
				modifiedAt < Date.parse(this.ifModifiedSince)
			) {
				return false;
			}
			if (
				this.ifUnmodifiedSince &&
				Date.parse(this.ifUnmodifiedSince) < modifiedAt
			) {
				return false;
			}
		}
...
export const conditionalResponse = createMiddleware<HonoCtx>(
	async (c, next) => {
		...
		if (
			!DESIRED_STATUS_CODES.has(c.res.status) ||
			condition.meetAllConditions(c.res.headers)
		) {
			return;
		}
