...
export const cfztAuth = () => {
	...
	return createMiddleware<HonoCtxWithAuth>(async (c, next) => {
		...
		if (!(await c.env.KV_AUTH.get(cacheKey, { cacheTtl: 3600 }))) {
			try {
				const jwt = await jose.jwtVerify<CfztJwtPayload>(token, JWKS, {
					issuer: ISS,
				});
				...
				console.error("M:a - failed to verify jwt", err);
				throw new LogicalError(HttpStatus.UNAUTHORIZED, {
					hrm: "M:a",
					msg: "invalid token",
				});
			}

			await c.env.KV_AUTH.put(cacheKey, new Date().toISOString(), {
				expiration: c.var.cfztJwt!.exp,
			});
		}
		// cached
		else {
