...
import { ResourcesMenu } from "@/app/components/ResourceMenu";
...
app.get(
	...
	async (c) => {
		...
		const manifest = [...(await getManifest(c, sub))].map(
		...
		return c.render(<ResourcesMenu sub={sub} ver={ver} manifest={manifest} />, {
			title: `Resource Menu: ${sub}`,
		});
	},
);
...
app.get(`/:sub/download/:ver/:name`, async (c) => {
	...
	return c.redirect(
		`https://${c.env.PUBLIC_STATIC_DOMAIN}/${sub}/${name}`,
		HttpStatus.MOVED_PERMANENTLY,
	);
});
...
app.get(
	...
	async (c) => {
		...
		const ret = await r2_retrive(c.env.R2_STATIC, key, {
